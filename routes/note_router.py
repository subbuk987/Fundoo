...
from db.redis import get_cached_notes, cache_user_notes, cache_user_labels

from models.user import User
from schema.note_schema import NoteCreate, NoteSuccessResponse, NoteRead, \
    LabelRead
...
async def create_note(note_data: NoteCreate,
    ...
    labels = NoteQueries.get_all_labels(
        db)
    await cache_user_labels(current_user.username,
                            [LabelRead.model_validate(l).model_dump() for l
                             in labels])
    await cache_user_notes(current_user.username,
                           [NoteRead.model_validate(n).model_dump() for n in
                            notes])
